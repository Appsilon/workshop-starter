...
server <- function(input, output) {
  ...
  output$iris_plot <- renderPlot({
    ...
    log_info("[event=x_axis][value={x_axis}]")
    log_info("[event=y_axis][value={y_axis}]")
    log_info("[event=plot_type][value={input$plot_type}]")
    log_info("[event=point_size][value={input$point_size}]")
